...
- (NSString *) description	{
	switch (type)	{
		...
		case OSCValString:
            ...
            return [NSString stringWithFormat:@"<OSCVal s %@>", (__bridge NSString *)(value)];
...
- (NSString *) lengthyDescription	{
	...
	switch (type)	{
		...
		case OSCValColor:
#if TARGET_OS_IPHONE
			return [NSString stringWithFormat:@"color %@",(__bridge id)value];
#else
...
- (id)initWithString:(NSString *)n {
    if (n == nil) {
        NSLog(@"\t\terr: %s - input string is nil", __func__);
        return nil;
    }
    if ((self = [super init])) {
        value = (__bridge_retained void *)([n copy]); // ✅ Retain and copy to avoid dangling pointer
        type = OSCValString;
        return self;
    }
    NSLog(@"\t\terr: %s - BAIL", __func__);
...
#endif
        value = (__bridge void *)(calibratedColor);
		type = OSCValColor;
		return self;
	}
	BAIL:
...
- (id) initArray	{
	self = [super init];
	if (self != nil)	{
        value = (__bridge void *)([[NSMutableArray alloc] initWithCapacity:0]);
		type = OSCValArray;
	}
	return self;
}
...
- (void)dealloc {
    switch (type) {
        ...
        case OSCValBlob:
            if (value != nil)
                CFRelease(value); // ✅ Properly release retained bridge
            break;
...
- (double) calculateDoubleValue	{
	...
	switch (type)	{
		...
		case OSCValColor:
#if TARGET_OS_IPHONE
			*comps = *(CGColorGetComponents([(__bridge UIColor *)value CGColor]));
#else
...
- (id) jsonValue	{
	...
	case OSCValColor:
		{
            UIColor		*tmpColor = [self colorValue];
			CGFloat		components[6];
            [tmpColor colorWithAlphaComponent:(CGFloat) 1.];
			//NSArray		*returnMe = @[
